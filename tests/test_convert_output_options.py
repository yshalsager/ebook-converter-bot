...
def test_doc_to_epub_extracts_with_anydoc_then_uses_calibre_by_default(
    ...
    async def run() -> None:
        ...
        output_file = input_file.with_suffix(".epub")
        monkeypatch.setattr(convert_utils.anydoc, "to_markdown", lambda _: "# مرحبا\n")
        ...
        async def fake_run(
            ...
            commands.append(command)
            assert Path(command[1]).read_text() == "# مرحبا\n"
            Path(command[2]).write_text("converted")
            return 0, ""
...
def test_doc_to_md_extracts_with_anydoc_then_uses_pandoc_gfm_input(
    tmp_path: Path, monkeypatch
) -> None:
    async def run() -> None:
        converter = Converter()
        ...
        input_file.write_bytes(b"legacy doc")
        output_file = input_file.with_suffix(".md")
        monkeypatch.setattr(convert_utils.anydoc, "to_markdown", lambda _: "# Heading\n")
        ...
        async def fake_run(
            ...
            commands.append(command)
            assert Path(command[1]).read_text() == "# Heading\n"
